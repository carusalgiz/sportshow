  ...
  <v-dialog v-model="fullInfoOpen" persistent max-width="800">
    <template v-slot:activator="{on, attrs}">
      <v-btn outlined text v-bind="attrs" v-on="on" large
        ...
        <div class="d-flex flex-column" style="width: 350px; font-size: 12px">
          <div class="d-flex justify-space-between w-100">
            <span class="mr-3">Статус заказа:</span>
            <span class="purple--text mb-0">{{statuses.filter(elem => elem.value === order.state)[0] != null ?
              statuses.filter(elem => elem.value === order.state)[0].name : ''}}</span>
          </div>
          <div class="d-flex justify-space-between w-100">
            <span class="mr-3">Статус оплаты:</span>
            <span class="purple--text mb-0">{{pay_state.filter(elem => elem.value === order.pay_state)[0] != null ?
              pay_state.filter(elem => elem.value === order.pay_state)[0].name : ''}}</span>
          </div>
        </div>
      </v-btn>
    </template>
...
<script>
    ...
    export default {
      ...
      mounted() {
        if (this.$route.params.id != null && this.$route.params.id === this.order.id) {
